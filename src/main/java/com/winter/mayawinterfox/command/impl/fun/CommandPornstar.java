	...
	public CommandPornstar() {
		super(new Command(
				...
				e -> {
					...
					MessageUtil.sendMessage(e.getChannel(), EmbedUtil.successEmbed(e.getGuild(), "pornstar-name", e.getAuthor().getName(), pre[new Random(e.getAuthor().getLongID()).nextInt(pre.length)], suf[new Random(e.getAuthor().getLongID()).nextInt(suf.length)]));
					return true;
				},
