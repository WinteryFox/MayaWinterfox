...
public class Caches {
	...
	public static GuildMeta getGuild(IGuild guild) {
		...
		return new GuildMeta(guild.getShard(), guild.getName(), guild.getLongID(), guild.getIcon() != null ? guild.getIcon() : "", guild.getOwnerLongID(), guild.getAFKChannel() != null ? guild.getAFKChannel().getLongID() : 0L, guild.getAFKTimeout(), guild.getRegion().getID(), guild.getVerificationLevel().ordinal(), prefixes.stream().map(v -> (String) v.get("prefix")).collect(Collectors.toSet()), autoroles.stream().map(v -> (Long) v.get("role")).collect(Collectors.toSet()), (String) settings.get(0).get("language"), (String) settings.get(0).get("welcome"), (String) settings.get(0).get("pm"), (boolean) settings.get(0).get("lvlup"), (boolean) settings.get(0).get("premium"), (boolean) settings.get(0).get("newguild"), (boolean) settings.get(0).get("permission"));
	}
